...
class TransaksiController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $query = Transaction::with(['items.obat', 'returns', 'returns.item.obat', 'returns.user', 'voidBy'])
            ->when($search, fn($q) => $q->where('kode', 'like', "%{$search}%"))
            ->when($status, fn($q) => $q->where('status', $status))
            ->where('user_id', Auth::id()); // Kasir only sees their own transactions
        ...
        return view('kasir.transaksi.index', compact('transaksis', 'search', 'startDate', 'endDate', 'status'));
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        return $this->exportService->exportTransaksi($query, includeStatusColumn: false);
    }
